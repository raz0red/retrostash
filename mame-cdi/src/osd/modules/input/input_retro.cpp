...
void retro_osd_interface::process_joystick_state(running_machine &machine)
{
   ...
   for (j = 0; j < RETRO_MAX_PLAYERS; j++)
   {
      ...
      /* Apply WRC input state pushed from JavaScript controller layer */
      {
         ...
         if (ws & INP_LEFT)  joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_LEFT]  = BUTTON_MAX;
         if (ws & INP_RIGHT) joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_RIGHT] = BUTTON_MAX;
         if (ws & INP_UP)    joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_UP]    = BUTTON_MAX;
         if (ws & INP_DOWN)  joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_DOWN]  = BUTTON_MAX;
         if (ws & INP_A)     joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_B] = BUTTON_MAX; /* button 0 */
         if (ws & INP_B)     joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_A] = BUTTON_MAX; /* button 1 */
         if (ws & INP_LBUMP) joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_B] = BUTTON_MAX; /* button 0 */
         if (ws & INP_RBUMP) joystickstate[j].button[RETRO_DEVICE_ID_JOYPAD_A] = BUTTON_MAX; /* button 1 */

         /* Right analog X → paddle, scaled so ~75% stick = full range */
         float paddle_scaled = wrc_input_state_analog[j][2] * (4.0f / 3.0f);
...
void retro_osd_interface::process_mouse_state(running_machine &machine)
{
	...
	for (i = 0; i < 1 /*RETRO_MAX_PLAYERS*/; i++)
	{
		...
		if (i == 0) {
			...
			int16_t wrc_mx = (int16_t)(rx * SPEED);
			int16_t wrc_my = (int16_t)(ry * SPEED);
...
public:
	...
	virtual void configure(osd::input_device &device)
	{
		...
		// paddle / spinner (right analog X)
		assignments.emplace_back(
				IPT_PADDLE,
				SEQ_TYPE_STANDARD,
				input_seq(make_code(ITEM_CLASS_ABSOLUTE, ITEM_MODIFIER_NONE, axis_ids[AXIS_RX])));
