...
void m68000_musashi_device::execute_run()
{
	...
	if(!m_stopped)
	{
		...
		if (m_address_error==1)
		{
			...
			{
				volatile int aerr1_code = setjmp(m_aerr_jmp_buf);
				if (aerr1_code == 0)
					m68ki_exception_address_error();
				else if (aerr1_code == 10)
				{
					m_address_error = 1;
					m_ppc = m_pc;
...
#ifdef __EMSCRIPTEN__
			{
			volatile int aerr2_code = setjmp(m_aerr_jmp_buf);
			if (aerr2_code == 10) { m_address_error = 1; goto check_address_error; }
			}
			{
#else
			try
