...
*/
void ym2151_update_one(void *chip, SAMP *bufL, SAMP *bufR, int length)
{
	int i, chn;
...
void ym_Reset() {
    if (PSG) free(PSG);
    if (ym_lbuf) free(ym_lbuf);
    if (ym_rbuf) free(ym_rbuf);

    PSG = ym2151_init(FREQ, RATE);
    ym2151_reset_chip(PSG);
    ym_lbuf = (SAMP*)malloc(sizeof(SAMP) * BUFFER_SIZE);
    ym_rbuf = (SAMP*)malloc(sizeof(SAMP) * BUFFER_SIZE);
}

uint8_t ym_registers[256] = {0};

void ym_SetReg(int r, int v) {
    if (r >= 0 && r < 256)
        ym_registers[r] = (uint8_t)v;
    ym2151_write_reg(PSG, r, v);
}

void ym_Generate(int length) {
    ym2151_update_one(PSG, ym_lbuf, ym_rbuf, length);
}

int ym_GetStatus() {
    return ym2151_read_status(PSG);
}