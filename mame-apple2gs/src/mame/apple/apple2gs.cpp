...
#ifdef WRC
extern "C" int wrc_get_cpu_speed();
#endif
...
#ifdef WRC
	// WRC: Get CPU speed from JS layer (feed property)
	int cpuSpeedSetting = wrc_get_cpu_speed();

	if (cpuSpeedSetting > 0) {
		static const int speeds[7] = { 4000000, 5000000, 7000000, 8000000, 10000000, 12000000, 16000000 };
		m_accel_present = true;
		// cpuSpeedSetting: 1=4MHz, 2=5MHz, 3=7MHz, 4=8MHz, 5=10MHz, 6=12MHz, 7=16MHz
		int idx = cpuSpeedSetting - 1;
		if (idx >= 0 && idx < 7) {
			m_accel_speed = speeds[idx];
			m_accel_fast = true;
			accel_full_speed();
		}
	}
