...
{
#ifdef WRC
	// WRC: Check if there's mouse/analog delta OR button change
	// Analog stick AND bumpers are handled in JS layer (pollControls in index.js)
	s32 const NewButton = wrc_buttons & 0x03;  // Buttons from JS (includes real mouse + bumpers)
	s32 const deltaX = (int16_t)wrc_mouse_x;
	s32 const deltaY = (int16_t)wrc_mouse_y;

	return (deltaX != 0) || (deltaY != 0) || (NewButton != m_lastbutton);
#else
...
void macadb_device::adb_accummouse(u8 *MouseX, u8 *MouseY )
{
...
#ifdef WRC
	// WRC: Use deltas directly from JS layer (includes mouse + analog stick)
	// Analog stick is handled in pollControls() in index.js, not here
	int deltaX = (int16_t)wrc_mouse_x;
	int deltaY = (int16_t)wrc_mouse_y;
	wrc_mouse_x = 0;
	wrc_mouse_y = 0;
	...
	// Use deltas directly as the mouse movement
	MouseCountX = deltaX;
	MouseCountY = deltaY;
#else
...
#ifdef WRC
	// Buttons are handled in JS layer (includes real mouse + bumpers)
	m_lastbutton = wrc_buttons & 0x03;
#else
