...
uint8_t a2bus_mouse_device::mcu_port_b_r()
{
	...
	// update the axes from WRC mouse delta
	const int16_t wrc_mx = (int16_t)wrc_mouse_x;
	const int16_t wrc_my = (int16_t)wrc_mouse_y;

	static int debug_counter = 0;
	if (++debug_counter % 60 == 0 || wrc_mx != 0 || wrc_my != 0 || wrc_buttons != 0)
	{
		printf("[WRC MOUSE] mcu_port_b_r: mx=%d my=%d buttons=0x%x count[0]=%d count[1]=%d\n",
			wrc_mx, wrc_my, wrc_buttons, m_count[XAXIS], m_count[YAXIS]);
	}
	...
	if (m_count[XAXIS])
	{
		...
		{
			m_count[XAXIS]++;
			if (m_port_b_in & X0) m_port_b_in &= ~X1;
		}
		else
		{
			m_count[XAXIS]--;
	...
	if (m_count[YAXIS])
	{
		...
		{
			m_count[YAXIS]++;
			if (m_port_b_in & Y1) m_port_b_in &= ~Y0;
		}
		else
		{
			m_count[YAXIS]--;
