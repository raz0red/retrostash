...
void es5503_device::sound_stream_update(sound_stream &stream)
{
	...
	// Output the mixed audio - mix_buffer is interleaved (sample 0 chan 0, sample 0 chan 1, sample 1 chan 0, etc.)
	for (i = 0; i < stream.samples(); i++)
	{
		for (int chan = 0; chan < m_output_channels; chan++)
		{
			stream.put_int(chan, i, m_mix_buffer[i * m_output_channels + chan], 32768*8);
		}
	}
}
