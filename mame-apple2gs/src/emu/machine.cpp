...
{
	// dump the full device tree before starting
	printf("WRC: device tree enumeration:\n");
	for (device_t &device : device_enumerator(root_device()))
		printf("WRC:   device: '%s' type='%s' started=%d\n", device.tag(), device.name(), device.started() ? 1 : 0);
	printf("WRC: end device tree\n");
		...
		for (device_t &device : device_enumerator(root_device()))
			if (!device.started())
			{
				...
				try
				{
					...
					// if device deferred (Emscripten: returned early instead of throwing)
					if (!device.started())
					{
						osd_printf_verbose("  (deferred; rescheduling)\n");
						failed_starts++;
					}
				}
