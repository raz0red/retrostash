...
#include <string.h>
#include <stdarg.h>
// chd.h intentionally not included - CHD types stubbed below for WRC builds
...
bool memory_maps_set = false;
bool libretro_supports_bitmasks = false;

/* Apple IIGS RAM buffer for RetroAchievements (64KB) */
static uint8_t apple2gs_ram_buffer[65536];
static bool apple2gs_ram_initialized = false;
...
static void check_variables(void)
{
   ...
   // Force Apple IIe settings — command line string passes system/media explicitly
   softlist_enable = false;
}
...
static void update_apple2gs_ram_buffer(void);
...
void retro_run(void)
{
   ...
   /* IIGS achievements disabled */

   /* Keep trying to set up memory maps until we get valid RAM */
   if (!memory_maps_set)
   {
      if (frame_count < 5)
         log_cb(RETRO_LOG_INFO, "[CHEEVOS] retro_run frame %d, calling setup_memory_maps\n", frame_count);
      setup_memory_maps();
   }
...
//FIXME: re-add way to handle OGL
   ...
   */
   ...
   /* IIGS achievements disabled - no achievement support for IIGS games currently */
...
void *find_mame_bank_base(offs_t start, address_space &space)
{
   ...
   if (!logged)
   {
      ...
      {
         log_cb(RETRO_LOG_INFO, "[CHEEVOS] Bank: %s base=%p\n",
            bank.first.c_str(), bank.second ? bank.second->base() : nullptr);
         bank_count++;
      }
      log_cb(RETRO_LOG_INFO, "[CHEEVOS] Total banks: %d\n", bank_count);

      int share_count = 0;
      for (auto &share : mame_machine_manager::instance()->machine()->memory().shares())
      {
         log_cb(RETRO_LOG_INFO, "[CHEEVOS] Share: %s ptr=%p size=%u\n",
            share.first.c_str(), share.second ? share.second->ptr() : nullptr,
            share.second ? (unsigned)share.second->bytes() : 0);
         share_count++;
      }
      log_cb(RETRO_LOG_INFO, "[CHEEVOS] Total shares: %d\n", share_count);
      logged = true;
   }
   ...
   for (auto &bank : mame_machine_manager::instance()->machine()->memory().banks())
   {
      if (bank.second && bank.second->base())
         return bank.second->base();
   }

   /* Try memory shares if no banks */
   for (auto &share : mame_machine_manager::instance()->machine()->memory().shares())
   {
      if (share.second && share.second->ptr())
         return share.second->ptr();
   }

   return NULL;
}

void update_apple2gs_ram_buffer()
{
   ...
   {
      if (memory.has_space(0))
      {
         auto &space = memory.space(0);
         /* Copy Apple IIGS RAM, skipping I/O region ($C000-$CFFF) to avoid side effects */
         for (offs_t addr = 0; addr < 0xC000; addr++)
            apple2gs_ram_buffer[addr] = space.read_byte(addr);
         /* Fill I/O region with zeros */
         for (offs_t addr = 0xC000; addr < 0xD000; addr++)
            apple2gs_ram_buffer[addr] = 0;
         /* Copy upper ROM region */
         for (offs_t addr = 0xD000; addr < 0x10000; addr++)
            apple2gs_ram_buffer[addr] = space.read_byte(addr);
         apple2gs_ram_initialized = true;
         return;
      }
   }
}
...
void *retro_get_memory_data(unsigned type)
{
   ...
   if (     type == RETRO_MEMORY_SYSTEM_RAM
         ...
         && mame_machine_manager::instance()->machine() != NULL)
   {
      /* Apple IIGS: use our RAM buffer that we update from address space */
      if (apple2gs_ram_initialized)
      {
         if (!logged)
         {
            log_cb(RETRO_LOG_INFO, "[CHEEVOS] Using Apple IIGS RAM buffer at %p size=65536\n", apple2gs_ram_buffer);
            logged = true;
         }
         return apple2gs_ram_buffer;
      }
   }

   /* Fallback to original AMH_RAM search for other machines */
   if (     type == RETRO_MEMORY_SYSTEM_RAM
         && mame_machine_manager::instance() != NULL
         && mame_machine_manager::instance()->machine() != NULL)
   {
...
size_t retro_get_memory_size(unsigned type)
{
   ...
   if (     type == RETRO_MEMORY_SYSTEM_RAM
         ...
         && mame_machine_manager::instance()->machine() != NULL)
   {
      /* Apple IIGS: return our buffer size */
      if (apple2gs_ram_initialized)
         return 65536;
   }
...
static void setup_memory_maps(void)
{
   ...
   /* Use existing retro_get_memory_data to get the main RAM pointer */
   void *ram_ptr = retro_get_memory_data(RETRO_MEMORY_SYSTEM_RAM);
   size_t ram_size = retro_get_memory_size(RETRO_MEMORY_SYSTEM_RAM);

   if (log_count++ < 5)
      log_cb(RETRO_LOG_INFO, "[CHEEVOS] RAM ptr=%p size=%u\n", ram_ptr, (unsigned)ram_size);

   if (ram_ptr && ram_size > 0)
   {
      struct retro_memory_descriptor descriptors[1];
      descriptors[0].flags  = RETRO_MEMDESC_SYSTEM_RAM;
      descriptors[0].ptr    = ram_ptr;
      descriptors[0].offset = 0;
      descriptors[0].start  = 0x000000;
      descriptors[0].select = 0;
      descriptors[0].disconnect = 0;
      descriptors[0].len    = ram_size;
      descriptors[0].addrspace = NULL;

      struct retro_memory_map mmap;
      mmap.descriptors = descriptors;
      mmap.num_descriptors = 1;
      environ_cb(RETRO_ENVIRONMENT_SET_MEMORY_MAPS, &mmap);
      memory_maps_set = true;
      log_cb(RETRO_LOG_INFO, "[CHEEVOS] Memory maps set: %u bytes at $000000\n", (unsigned)ram_size);
   }
}
...
void wrc_on_set_options(int opts) {
   if (opts & OPT12) {
      ...
      char* flop3Path = (char*)malloc(len);
      char* flop4Path = (char*)malloc(len);
      memset(flop1Path, 0, len);
      memset(flop2Path, 0, len);
      memset(flop3Path, 0, len);
      memset(flop4Path, 0, len);

      EM_ASM({
         const p1 = window.emulator.getFlop1Path ? window.emulator.getFlop1Path() : '';
         const p2 = window.emulator.getFlop2Path ? window.emulator.getFlop2Path() : '';
         const p3 = window.emulator.getFlop3Path ? window.emulator.getFlop3Path() : '';
         const p4 = window.emulator.getFlop4Path ? window.emulator.getFlop4Path() : '';
         const s1 = new Uint8Array(window.Module.HEAP8.buffer, $0, $1);
         const s2 = new Uint8Array(window.Module.HEAP8.buffer, $2, $3);
         const s3 = new Uint8Array(window.Module.HEAP8.buffer, $4, $5);
         const s4 = new Uint8Array(window.Module.HEAP8.buffer, $6, $7);
         for (let i = 0; i < p1.length; i++) s1[i] = p1.charCodeAt(i);
         for (let i = 0; i < p2.length; i++) s2[i] = p2.charCodeAt(i);
         for (let i = 0; i < p3.length; i++) s3[i] = p3.charCodeAt(i);
         for (let i = 0; i < p4.length; i++) s4[i] = p4.charCodeAt(i);
      }, flop1Path, len, flop2Path, len, flop3Path, len, flop4Path, len);
         ...
         for (device_image_interface &image : iter) {
            if (strstr(image.image_type_name(), "floppy") != NULL) {
               ...
               if (brief == "flop1") {
                  ...
                  image.unload();
                  if (flop3Path[0] != '\0') image.load(std::string_view(flop3Path));
               } else if (brief == "flop4") {
                  image.unload();
                  if (flop4Path[0] != '\0') image.load(std::string_view(flop4Path));
               }
            }
         }
      }
...
void wrc_disk_activity_set(int active) {
    EM_ASM({
        if (window.emulator && window.emulator.onDiskActivity) {
            window.emulator.onDiskActivity($0);
        }
    }, active);
}
int wrc_get_cpu_speed() {
    return EM_ASM_INT({
        if (window.emulator && window.emulator.getCpuSpeed) {
            return window.emulator.getCpuSpeed();
        }
        return 0;
    });
}
...
// CHD stubs - chd_file already forward declared in utilfwd.h
typedef void chd_header;
typedef int chd_error;
#define CHDERR_UNSUPPORTED_FORMAT 8
const chd_header *chd_get_header(chd_file *chd) { return 0; }
chd_error chd_get_metadata(chd_file *chd, uint32_t searchtag, uint32_t searchindex, void *output, uint32_t outputlen, uint32_t *resultlen, uint32_t *resulttag, uint8_t *resultflags) { return CHDERR_UNSUPPORTED_FORMAT; }
chd_error chd_open(const char *filename, int mode, chd_file *parent, chd_file **chd) { return CHDERR_UNSUPPORTED_FORMAT; }
void chd_close(chd_file *chd) {}
chd_error chd_read(chd_file *chd, uint32_t hunknum, void *buffer) { return CHDERR_UNSUPPORTED_FORMAT; }
}
#else
extern "C" void wrc_disk_activity_set(int active) {}
#endif
