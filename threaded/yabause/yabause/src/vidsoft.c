...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      // 1. ATOMIC LOAD: Force read from main memory, ignoring cache.
      // Checks if 'need_draw' is non-zero safely.
      if (__atomic_load_n(&vidsoft_vdp1_thread_context.need_draw, __ATOMIC_SEQ_CST))
      {
         // Reset flag
         __atomic_store_n(&vidsoft_vdp1_thread_context.need_draw, 0, __ATOMIC_SEQ_CST);

         // Do the work
         Vdp1DrawCommands(vidsoft_vdp1_thread_context.ram, &vidsoft_vdp1_thread_context.regs, vidsoft_vdp1_thread_context.back_framebuffer);
         memcpy(vdp1backframebuffer, vidsoft_vdp1_thread_context.back_framebuffer, 0x40000);

         // 2. ATOMIC STORE: Force write to main memory so Main Thread sees it immediately.
         __atomic_store_n(&vidsoft_vdp1_thread_context.draw_finished, 1, __ATOMIC_SEQ_CST);
      }
...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      // 1. Efficient Wait (Futex)
      // Check if need_draw is 0.
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.need_draw, __ATOMIC_SEQ_CST) == 0)
      {
         // This puts the thread to sleep efficiently at the OS level
         // It wakes up ONLY when the value at this address changes via emscripten_futex_wake
         // Note: The timeout is INFINITY, but you can set a ms value if you prefer.
         emscripten_futex_wait(
            &vidsoft_vdp1_thread_context.need_draw, // Address
            0,                                      // Expected value (wait while it is this)
            INFINITY                                // Timeout
         );
      }
      ...
      // OPTIONAL: If the Main thread used emscripten_futex_wait, you would wake it here.
      // But since Main thread usually can't block-wait, the atomic store is enough
      // combined with the loop in the Main thread function above.
   }
}
#endif
...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      // 1. Wait for work (Spin with Yield)
      // This completely replaces YabThreadSleep() to avoid "Lost Wakeup" bugs.
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.need_draw, __ATOMIC_SEQ_CST) == 0)
      {
         // Pause briefly to let Main Thread run and save battery/CPU
         // sched_yield() or emscripten_thread_sleep(1) is ideal here.
         //YabThreadYield();
         YabThreadSleep();
      }
...
void VidsoftWaitForVdp1Thread()
{
   if (vidsoft_vdp1_thread_enabled)
   {
      // Use Atomic Load to verify the value from main memory (bypassing cache)
      // Check for 0 (Not Finished)
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.draw_finished, __ATOMIC_SEQ_CST) == 0)
      {
         // CRITICAL: Yield to the browser/system.
         // This allows the Worker Thread to execute and the browser to process
         // any proxy messages (printf, malloc proxies, etc) causing the deadlock.
         //YabThreadYield();
         YabThreadSleep();
      }
   }
...
void VidsoftWaitForVdp1Thread()
{
   if (vidsoft_vdp1_thread_enabled)
   {
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.draw_finished, __ATOMIC_SEQ_CST) == 0)
      {
         // 1. Process any work the Worker thread might be asking us to do.
         // This prevents the Proxy Deadlock.
         emscripten_main_thread_process_queued_calls();

         // 2. Sleep briefly to yield CPU to the browser (don't burn 100% CPU).
         // 1ms is usually enough to let the browser breathe.
...
void VIDSoftVdp1DrawStart()
{
   if (vidsoft_vdp1_thread_enabled)
   {
      // 1. Wait for previous frame (ensure you applied the fix to this function too!)
      VidsoftWaitForVdp1Thread();
      ...
      // Reset finished flag atomically so Main Thread knows to wait
      __atomic_store_n(&vidsoft_vdp1_thread_context.draw_finished, 0, __ATOMIC_SEQ_CST);

      // Signal thread to start atomically (Force write to main memory)
      __atomic_store_n(&vidsoft_vdp1_thread_context.need_draw, 1, __ATOMIC_SEQ_CST);

      // TODO: just added
      // WAKE UP THE WORKER
      emscripten_futex_wake(&vidsoft_vdp1_thread_context.need_draw, 1);
...
{ \
   for (;;) \
   { \
      while (__atomic_load_n(&vidsoft_thread_context.need_draw[LAYER], __ATOMIC_SEQ_CST) == 0) { \
         YabThreadYield(); \
      } \
      __atomic_store_n(&vidsoft_thread_context.need_draw[LAYER], 0, __ATOMIC_SEQ_CST); \
      FUNC(vidsoft_thread_context.lines, &vidsoft_thread_context.regs, vidsoft_thread_context.ram, vidsoft_thread_context.color_ram, vidsoft_thread_context.cell_scroll_data); \
      __atomic_store_n(&vidsoft_thread_context.draw_finished[LAYER], 1, __ATOMIC_SEQ_CST); \
   } \
}
...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      ...
      // 3. Do the work
      // Optimization: Draw directly to global framebuffer, read from local RAM snapshot
      Vdp1DrawCommands(vidsoft_vdp1_thread_context.ram, &vidsoft_vdp1_thread_context.regs, vdp1backframebuffer);
...
void VidsoftSpriteThread(void * data)
{
   for (;;)
   {
      while (__atomic_load_n(&vidsoft_thread_context.need_draw[TITAN_SPRITE], __ATOMIC_SEQ_CST) == 0) {
         YabThreadYield();
      }
      __atomic_store_n(&vidsoft_thread_context.need_draw[TITAN_SPRITE], 0, __ATOMIC_SEQ_CST);
      VidsoftDrawSprite(&vidsoft_thread_context.regs, sprite_window_mask, vdp1frontframebuffer, vidsoft_thread_context.ram, Vdp1Regs,vidsoft_thread_context.lines, vidsoft_thread_context.color_ram);
      __atomic_store_n(&vidsoft_thread_context.draw_finished[TITAN_SPRITE], 1, __ATOMIC_SEQ_CST);
   }
}
...
void VIDSoftVdp1DrawStart()
{
   if (vidsoft_vdp1_thread_enabled)
   {
      ...
      // Optimization: Do NOT copy input buffer.
      // memcpy(vidsoft_vdp1_thread_context.back_framebuffer, vdp1backframebuffer, 0x40000);

      // Call body on the GLOBAL buffer so it gets erased properly before drawing
      VIDSoftVdp1DrawStartBody(&vidsoft_vdp1_thread_context.regs, vdp1backframebuffer);
...
void VIDSoftVdp2DrawEnd(void)
{
   if (vidsoft_num_layer_threads > 0)
   {
      int i;
      for (i = 0; i < 6; i++) {
         while (__atomic_load_n(&vidsoft_thread_context.draw_finished[i], __ATOMIC_SEQ_CST) == 0)
         {
            YabThreadYield();
         }
      }
...
void VidsoftStartLayerThread(int * layer_priority, int * draw_priority_0, int * num_threads_dispatched, int which_layer, void(*layer_func) (Vdp2* lines, Vdp2* regs, u8* ram, u8* color_ram, struct CellScrollData * cell_data))
{
   if (layer_priority[which_layer] > 0 || draw_priority_0[which_layer])
   {
      if (*num_threads_dispatched < vidsoft_num_layer_threads)
      {
         __atomic_store_n(&vidsoft_thread_context.draw_finished[which_layer], 0, __ATOMIC_SEQ_CST);
         __atomic_store_n(&vidsoft_thread_context.need_draw[which_layer], 1, __ATOMIC_SEQ_CST);
         YabThreadWake(YAB_THREAD_VIDSOFT_LAYER_NBG3 + which_layer);
         *num_threads_dispatched = *num_threads_dispatched + 1;
      }
      else
...
void VIDSoftVdp2DrawScreens(void)
{
   ...
   if (CanUseSpriteThread() && vidsoft_num_layer_threads > 0)
   {
      __atomic_store_n(&vidsoft_thread_context.draw_finished[TITAN_SPRITE], 0, __ATOMIC_SEQ_CST);
      __atomic_store_n(&vidsoft_thread_context.need_draw[TITAN_SPRITE], 1, __ATOMIC_SEQ_CST);
      YabThreadWake(YAB_THREAD_VIDSOFT_LAYER_SPRITE);
      num_threads_dispatched++;
   }
   else
...
#include <emscripten.h>
#include <emscripten/threading.h> // Required for emscripten_thread_sleep
...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.need_draw, __ATOMIC_SEQ_CST) == 0)
      {
         // Sleep 0 to yield to the browser
         emscripten_thread_sleep(0);
      }

      __atomic_store_n(&vidsoft_vdp1_thread_context.need_draw, 0, __ATOMIC_SEQ_CST);

      // Render Polygons: Use SNAPSHOT RAM (correctness) and GLOBAL Framebuffer (speed)
      Vdp1DrawCommands(vidsoft_vdp1_thread_context.ram, &vidsoft_vdp1_thread_context.regs, vdp1backframebuffer);
...
void VIDSoftVdp1DrawStart()
{
   if (vidsoft_vdp1_thread_enabled)
   {
      ...
      // OPTIMIZATION: Do not copy the framebuffer in (save 256KB copy)
      // memcpy(vidsoft_vdp1_thread_context.back_framebuffer, vdp1backframebuffer, 0x40000);

      // Call body on GLOBAL buffer so it gets erased properly
      VIDSoftVdp1DrawStartBody(&vidsoft_vdp1_thread_context.regs, vdp1backframebuffer);
...
void VidsoftVdp1Thread(void* data)
{
   for (;;)
   {
      while (__atomic_load_n(&vidsoft_vdp1_thread_context.need_draw, __ATOMIC_SEQ_CST) == 0)
      {
         // Wait for work (Spin with Yield)
         // This completely replaces YabThreadSleep() to avoid "Lost Wakeup" bugs.
         // emscripten_thread_sleep(0) allows the main thread to run immediately.
         emscripten_thread_sleep(0);
      }

      __atomic_store_n(&vidsoft_vdp1_thread_context.need_draw, 0, __ATOMIC_SEQ_CST);

      // Render: Use Context RAM (Snapshot) but Global Framebuffer (Zero-Copy Output)
      Vdp1DrawCommands(vidsoft_vdp1_thread_context.ram, &vidsoft_vdp1_thread_context.regs, vdp1backframebuffer);
...
void VIDSoftVdp2DrawEnd(void)
{
   if (vidsoft_num_layer_threads > 0)
   {
      ...
      for (i = 0; i < 6; i++) {
         while (__atomic_load_n(&vidsoft_thread_context.draw_finished[i], __ATOMIC_SEQ_CST) == 0)
         {
            // YIELD: Wait for VDP2 threads (Backgrounds/Sprites).
            // Using a simple yield loop prevents the periodic "hitching"
            // caused by slow sleep/wake cycles.
            YabThreadYield();
         }
      }
   }
