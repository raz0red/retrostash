...
void Emulator::init()
{
	printf("[WRC-DEBUG] Emulator::init() enter\n"); fflush(stdout);
	...
	printf("[WRC-DEBUG] before libGDR_init\n"); fflush(stdout);
	libGDR_init();
	printf("[WRC-DEBUG] before pvr::init\n"); fflush(stdout);
	pvr::init();
	printf("[WRC-DEBUG] before aica::init\n"); fflush(stdout);
	aica::init();
	printf("[WRC-DEBUG] before mem_Init\n"); fflush(stdout);
	mem_Init();
	printf("[WRC-DEBUG] before reios_init\n"); fflush(stdout);
	reios_init();
	printf("[WRC-DEBUG] after reios_init\n"); fflush(stdout);
	...
	printf("[WRC-DEBUG] before recompiler->Init\n"); fflush(stdout);
	recompiler->Init();
	printf("[WRC-DEBUG] after recompiler->Init\n"); fflush(stdout);
	...
	printf("[WRC-DEBUG] before interpreter->Init\n"); fflush(stdout);
	interpreter->Init();
	printf("[WRC-DEBUG] after interpreter->Init, Emulator::init() done\n"); fflush(stdout);
	state = Init;
}
...
{
	printf("[WRC-DEBUG] Emulator::loadGame() enter, path=%s\n", path == nullptr ? "(nil)" : path); fflush(stdout);
	init();
	printf("[WRC-DEBUG] init() returned, entering try block\n"); fflush(stdout);
	try {
		...
		printf("[WRC-DEBUG] setPlatform done, before mem_map_default\n"); fflush(stdout);
		mem_map_default();
		printf("[WRC-DEBUG] mem_map_default done\n"); fflush(stdout);

		config::Settings::instance().reset();
		config::Settings::instance().load(false);
		printf("[WRC-DEBUG] before dc_reset\n"); fflush(stdout);
		dc_reset(true);
		printf("[WRC-DEBUG] after dc_reset\n"); fflush(stdout);
		...
		if (settings.platform.isConsole())
		{
			...
			else
			{
				...
				printf("[WRC-DEBUG] content extension=%s\n", extension.c_str()); fflush(stdout);
				if (extension != "elf")
				{
					printf("[WRC-DEBUG] before gdr::initDrive path=%s\n", settings.content.path.c_str()); fflush(stdout);
					bool driveOk = gdr::initDrive(settings.content.path);
					printf("[WRC-DEBUG] after gdr::initDrive, result=%d\n", (int)driveOk); fflush(stdout);
					if (driveOk)
					{
						printf("[WRC-DEBUG] before loadGameSpecificSettings\n"); fflush(stdout);
						loadGameSpecificSettings();
						printf("[WRC-DEBUG] after loadGameSpecificSettings\n"); fflush(stdout);
						if (config::UseReios || !nvmem::loadFiles())
						{
							printf("[WRC-DEBUG] before nvmem::loadHle\n"); fflush(stdout);
							nvmem::loadHle();
							printf("[WRC-DEBUG] after nvmem::loadHle\n"); fflush(stdout);
		...
		printf("[WRC-DEBUG] before maple device setup\n"); fflush(stdout);
		if (!settings.naomi.slave)
		{
			mcfg_DestroyDevices();
			printf("[WRC-DEBUG] before mcfg_CreateDevices\n"); fflush(stdout);
			mcfg_CreateDevices();
			printf("[WRC-DEBUG] after mcfg_CreateDevices\n"); fflush(stdout);
...
#endif
		printf("[WRC-DEBUG] before cheatManager.reset\n"); fflush(stdout);
		cheatManager.reset(settings.content.gameId);
		printf("[WRC-DEBUG] after cheatManager.reset\n"); fflush(stdout);
		...
		loadGameSpecificSettings();
		printf("[WRC-DEBUG] before NetworkHandshake::init\n"); fflush(stdout);
		NetworkHandshake::init();
		printf("[WRC-DEBUG] after NetworkHandshake::init\n"); fflush(stdout);
		settings.input.fastForwardMode = false;
		EventManager::event(Event::Start);
		printf("[WRC-DEBUG] after EventManager::event(Start)\n"); fflush(stdout);
		if (!settings.content.path.empty())
		{
#ifndef LIBRETRO
			if (config::GGPOEnable)
		...
		printf("[WRC-DEBUG] Emulator::loadGame() reached end, state=Loaded\n"); fflush(stdout);
		state = Loaded;
	} catch (...) {
		printf("[WRC-DEBUG] Emulator::loadGame() caught exception, state=Error\n"); fflush(stdout);
		state = Error;
		throw;
	}
}
