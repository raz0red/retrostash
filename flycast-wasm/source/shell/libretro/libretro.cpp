...
{
	printf("[WRC-DEBUG] retro_load_game() enter\n"); fflush(stdout);
#if defined(IOS)
	bool can_jit;
	...
	printf("[WRC-DEBUG] before update_variables\n"); fflush(stdout);
	update_variables(true);
	printf("[WRC-DEBUG] after update_variables\n"); fflush(stdout);
	...
	printf("[WRC-DEBUG] before boot_to_bios branch, boot_to_bios=%d disk_paths.size()=%d\n", (int)boot_to_bios, (int)disk_paths.size()); fflush(stdout);
	if (boot_to_bios) {
		game_data.clear();
	}
	...
	{
		printf("[WRC-DEBUG] entered else block, game->path=%s\n", game->path); fflush(stdout);
		char disk_label[PATH_MAX];
		disk_label[0] = '\0';

		printf("[WRC-DEBUG] before disk_paths.push_back\n"); fflush(stdout);
		disk_paths.push_back(game->path);
		printf("[WRC-DEBUG] after disk_paths.push_back, before fill_short_pathname_representation\n"); fflush(stdout);

		fill_short_pathname_representation(disk_label, game->path, sizeof(disk_label));
		printf("[WRC-DEBUG] after fill_short_pathname_representation, disk_label=%s\n", disk_label); fflush(stdout);
		disk_labels.push_back(disk_label);
		printf("[WRC-DEBUG] after disk_labels.push_back\n"); fflush(stdout);

		game_data = game->path;
		printf("[WRC-DEBUG] after game_data assignment\n"); fflush(stdout);
	}
	printf("[WRC-DEBUG] disk_paths/game_data set, game_data=%s\n", game_data.c_str()); fflush(stdout);
	...
	printf("[WRC-DEBUG] data dir handled, before GET_PREFERRED_HW_RENDER\n"); fflush(stdout);

	u32 preferred;
	if (!environ_cb(RETRO_ENVIRONMENT_GET_PREFERRED_HW_RENDER, &preferred))
		preferred = RETRO_HW_CONTEXT_DUMMY;
	printf("[WRC-DEBUG] preferred hw render=%d\n", (int)preferred); fflush(stdout);
	...
	{
		printf("[WRC-DEBUG] before set_opengl_hw_render(preferred)\n"); fflush(stdout);
		foundRenderApi = set_opengl_hw_render(preferred);
		printf("[WRC-DEBUG] after set_opengl_hw_render(preferred)=%d\n", (int)foundRenderApi); fflush(stdout);
	}
	else if (preferred == RETRO_HW_CONTEXT_VULKAN)
	{
		printf("[WRC-DEBUG] before set_vulkan_hw_render (preferred branch)\n"); fflush(stdout);
		foundRenderApi = set_vulkan_hw_render();
		printf("[WRC-DEBUG] after set_vulkan_hw_render=%d\n", (int)foundRenderApi); fflush(stdout);
	}
	else if (preferred == RETRO_HW_CONTEXT_DIRECT3D)
	{
		printf("[WRC-DEBUG] before set_dx11_hw_render (preferred branch)\n"); fflush(stdout);
		foundRenderApi = set_dx11_hw_render();
		printf("[WRC-DEBUG] after set_dx11_hw_render=%d\n", (int)foundRenderApi); fflush(stdout);
	}
	else
	{
		// fallback when not supported (or auto-switching disabled), let's try all supported drivers
		printf("[WRC-DEBUG] fallback: before set_dx11_hw_render\n"); fflush(stdout);
		foundRenderApi = set_dx11_hw_render();
		printf("[WRC-DEBUG] fallback: after set_dx11_hw_render=%d\n", (int)foundRenderApi); fflush(stdout);
		if (!foundRenderApi) {
			printf("[WRC-DEBUG] fallback: before set_vulkan_hw_render\n"); fflush(stdout);
			foundRenderApi = set_vulkan_hw_render();
			printf("[WRC-DEBUG] fallback: after set_vulkan_hw_render=%d\n", (int)foundRenderApi); fflush(stdout);
		}
#if defined(HAVE_OPENGLES)
		if (!foundRenderApi) {
			printf("[WRC-DEBUG] fallback: before set_opengl_hw_render(GLES3)\n"); fflush(stdout);
			foundRenderApi = set_opengl_hw_render(RETRO_HW_CONTEXT_OPENGLES3);
			printf("[WRC-DEBUG] fallback: after set_opengl_hw_render(GLES3)=%d\n", (int)foundRenderApi); fflush(stdout);
		}
		if (!foundRenderApi) {
			printf("[WRC-DEBUG] fallback: before set_opengl_hw_render(GLES2)\n"); fflush(stdout);
			foundRenderApi = set_opengl_hw_render(RETRO_HW_CONTEXT_OPENGLES2);
			printf("[WRC-DEBUG] fallback: after set_opengl_hw_render(GLES2)=%d\n", (int)foundRenderApi); fflush(stdout);
		}
#else
		if (!foundRenderApi)
	...
	printf("[WRC-DEBUG] HW render negotiation complete, foundRenderApi=%d\n", (int)foundRenderApi); fflush(stdout);
	...
	config::ScreenStretching = 100;
	printf("[WRC-DEBUG] retro_load_game: before loadGame()\n"); fflush(stdout);
	if (!loadGame())
	{
		printf("[WRC-DEBUG] retro_load_game: loadGame() returned false\n"); fflush(stdout);
		return false;
	}
	printf("[WRC-DEBUG] retro_load_game: loadGame() returned true\n"); fflush(stdout);
	...
	printf("[WRC-DEBUG] retro_load_game: before card_reader::readerAvailable\n"); fflush(stdout);
	haveCardReader = card_reader::readerAvailable();
	printf("[WRC-DEBUG] retro_load_game: before dreampotato::update\n"); fflush(stdout);
	dreampotato::update();
	printf("[WRC-DEBUG] retro_load_game: before refresh_devices\n"); fflush(stdout);
	refresh_devices(true);

	// System may have changed - have to update hidden core options
	set_variable_visibility();

	printf("[WRC-DEBUG] retro_load_game: returning true (fully complete)\n"); fflush(stdout);
	return true;
}
