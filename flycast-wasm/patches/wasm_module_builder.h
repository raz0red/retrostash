...
public:
	...
	void endSection() {
		...
		// Closing the code section is the moment all branch-hint offsets are
		// known — splice the hint section in front of it (spec: the hint
		// section must PRECEDE code).
		if (currentSectionId == WASM_SEC_CODE)
			finalizeBranchHints();
	}
	...
	// N exports named <prefix>0..<prefix>N-1 for consecutive func indices —
	// batch modules (many block functions in one Module).
	void emitExportSectionMulti(const char* prefix, u32 firstFuncIdx, u32 count) {
		beginSection(WASM_SEC_EXPORT);
		emitLEB128(count);
		char name[16];
	...
	// br_table: jump to labels[idx] (stack: i32 idx), or defaultDepth if out
	// of range. Region modules use this for O(1) member dispatch.
	void op_br_table(const u32* depths, u32 count, u32 defaultDepth) {
		emitByte(0x0E);
		emitLEB128(count);
		for (u32 i = 0; i < count; i++) emitLEB128(depths[i]);
		emitLEB128(defaultDepth);
	}

	// --- Branch hinting (custom section "metadata.code.branch_hint") ---
	//
	// ★ BRANCH HINTS (2026-07-24, perf-headroom campaign): mark guard
	// slow-paths (fastmem import fallback, SMC guard fail, interrupt exits)
	// unlikely so engines lay hot code contiguously. Universal in browsers
	// since ~Feb 2026 (Firefox last); engines that don't know the section
	// ignore it — and engines that DO know it silently drop malformed or
	// mis-offset hints, so there is no crash mode, only silent no-op.
	// Encoding per the phase-5 proposal: the section must PRECEDE the code
	// section, but offsets are only known while emitting code — so hints
	// are collected during emission and the section is spliced in at
	// finalizeBranchHints(). Offsets are relative to the first byte of the
	// function body (the locals declaration, = funcBodyStart). Function
	// indices are module-wide (imports first): setHintFuncIndexBase().
	// Call hintNextBranch*() IMMEDIATELY before op_if()/op_br_if().
	// Finalize runs automatically when the code section's endSection() fires.

	void setHintFuncIndexBase(u32 importFuncCount) { hintFuncBase = importFuncCount; }

	void hintNextBranchUnlikely() { recordHint(0x00); }
	void hintNextBranchLikely()   { recordHint(0x01); }

	// Splice the collected hints in front of the code section. Invoked by
	// endSection() when the code section closes. No-op if nothing hinted.
	void finalizeBranchHints() {
		if (hints.empty())
			return;
		std::vector<u8> sec;
		auto leb = [&sec](u32 v) {
		...
		// Payload: name + vec(funchints); funchints = funcidx vec(hint);
		// hint = offset:LEB 0x01(payload len) value:byte. Hints arrive in
		// emission order, which is ascending (funcIdx, offset) by
		// construction — required by the spec.
		static const char name[] = "metadata.code.branch_hint";
		leb((u32)(sizeof(name) - 1));
		sec.insert(sec.end(), name, name + sizeof(name) - 1);
		u32 funcCount = 0;
		for (size_t i = 0; i < hints.size(); )
		{
			size_t j = i;
			while (j < hints.size() && hints[j].funcIdx == hints[i].funcIdx)
				j++;
			funcCount++;
			i = j;
		}
		...
		for (size_t i = 0; i < hints.size(); )
		{
			...
			for (size_t k = i; k < j; k++) {
				leb(hints[k].offset);
				sec.push_back(0x01);
				sec.push_back(hints[k].value);
			}
			i = j;
		}
		// Assemble the full section: id 0 + LEB(size) + payload, inserted
		// immediately before the code section.
		std::vector<u8> full;
		full.push_back(0x00);
		u32 sz = (u32)sec.size();
		do {
		...
		full.insert(full.end(), sec.begin(), sec.end());
		bytes.insert(bytes.begin() + codeSectionPos, full.begin(), full.end());
		hints.clear();
	}
...
private:
	struct BranchHint { u32 funcIdx; u32 offset; u8 value; };

	void recordHint(u8 value) {
		// Offset of the branch opcode about to be emitted, relative to the
		// function body start. hintFuncOrdinal is 1-based after
		// beginFuncBody().
		hints.push_back({ hintFuncBase + hintFuncOrdinal - 1,
		                  (u32)bytes.size() - funcBodyStart, value });
	}

	std::vector<u8> bytes;
	std::vector<BranchHint> hints;
	u32 hintFuncBase = 0;
	u32 hintFuncOrdinal = 0;
	u32 codeSectionPos = 0;
	u8 currentSectionId = 0;
