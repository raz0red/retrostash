...
#ifdef WRC
      int isPal = EM_ASM_INT({
         return window.emulator.isPal();
      });
      if (isPal) {
         FCEUD_RegionOverride(2);
         audio_video_updated = 2;
      }
#endif
   }
...
#else
            if (player < 4) {
               int controller = wrc_input_state[player];
               if (controller & INP_RIGHT) input_buf |= JOY_RIGHT;
               if (controller & INP_LEFT) input_buf |= JOY_LEFT;
               if (controller & INP_DOWN) input_buf |= JOY_DOWN;
               if (controller & INP_UP) input_buf |= JOY_UP;
               if (controller & INP_A) input_buf |= JOY_B;
               if (controller & INP_Y) input_buf |= JOY_B;
               if (controller & INP_B) input_buf |= JOY_A;
               if (controller & INP_X) input_buf |= JOY_A;
               if (controller & INP_START) input_buf |= JOY_START;
               if (controller & INP_SELECT) input_buf |= JOY_SELECT;
            }
#endif
         }
         else
