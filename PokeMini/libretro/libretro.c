...
#ifdef WRC
	sprintf(CommandLine.eeprom_file, "%s%c%s.eep", g_save_dir, slash, "pokemini");
#endif
}
...
static void handlekeyevents(void)
{
	...
	// D-Pad
	JoystickButtonsEvent(PM_BUTTON_RIGHT,  (controller & INP_RIGHT));
	JoystickButtonsEvent(PM_BUTTON_LEFT,   (controller & INP_LEFT));
	JoystickButtonsEvent(PM_BUTTON_DOWN,   (controller & INP_DOWN));
	JoystickButtonsEvent(PM_BUTTON_UP,     (controller & INP_UP));

	// Face buttons (Nintendo-style logic on Xbox)
	JoystickButtonsEvent(PM_BUTTON_B,      (controller & INP_A)); // Xbox A → Retro B
	JoystickButtonsEvent(PM_BUTTON_A,      (controller & INP_B)); // Xbox B → Retro A

	// Shoulders
	JoystickButtonsEvent(PM_BUTTON_SHAKE,  (controller & INP_LBUMP)); // LB
	JoystickButtonsEvent(PM_BUTTON_C,      (controller & INP_RBUMP)); // RB

	// Start and Select
	// SELECT (Back button) is mapped to POWER per RetroArch style
	JoystickButtonsEvent(PM_BUTTON_POWER,  (controller & INP_SELECT));
#else
	unsigned i;
