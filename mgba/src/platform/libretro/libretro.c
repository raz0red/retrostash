...
void retro_run(void) {
...
#ifdef WRC
	/* --- WebRcade / Xbox-to-emulator input mapping --- */

	/* Buttons */
	if (wrc_input_state[0] & INP_B)      keys |= 1 << 0;  // Retro A
	if (wrc_input_state[0] & INP_X)      keys |= 1 << 0;  // Retro A
	if (wrc_input_state[0] & INP_A)      keys |= 1 << 1;  // Retro B
	if (wrc_input_state[0] & INP_Y)      keys |= 1 << 1;  // Retro B
	...
	/* Shoulder / triggers */
	if (wrc_input_state[0] & INP_RBUMP)  keys |= 1 << 8;  // R
	if (wrc_input_state[0] & INP_LBUMP)  keys |= 1 << 9;  // L
#else
	if (useBitmasks) {
...
#ifdef WRC
				EM_ASM({ window.emulator.audioCallback($0, $1); }, audioSampleBuffer, samplesProduced);
#else
