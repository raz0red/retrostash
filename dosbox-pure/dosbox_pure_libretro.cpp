...
void retro_run(void)
{
	...
	if (dbp_mouse_input != 'f')
	{
...
#else
		int16_t movx = wrc_mouse_x;
		int16_t movy = wrc_mouse_y;
		int16_t absx = 0;
		int16_t absy = 0;
		...
		static int left_down = 0;   // 0
		static int right_down = 0;  // 1
		static int middle_down = 0; // 2

   		if (wrc_buttons & MOUSE_LEFT) {
			if (!left_down) {
				left_down = 1;
				DBP_QueueEvent(DBPET_MOUSEDOWN, 0);
			}
		} else if (left_down) {
			left_down = 0;
			DBP_QueueEvent(DBPET_MOUSEUP, 0);
		}

		if (wrc_buttons & MOUSE_RIGHT) {
			if (!right_down) {
				right_down = 1;
				DBP_QueueEvent(DBPET_MOUSEDOWN, 1);
			}
		} else if (right_down) {
			right_down = 0;
			DBP_QueueEvent(DBPET_MOUSEUP, 1);
		}

		if (wrc_buttons & MOUSE_MIDDLE) {
			if (!middle_down) {
				middle_down = 1;
				DBP_QueueEvent(DBPET_MOUSEDOWN, 2);
			}
		} else if (middle_down) {
			middle_down = 0;
			DBP_QueueEvent(DBPET_MOUSEUP, 2);
		}
