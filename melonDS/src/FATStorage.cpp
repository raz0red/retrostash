...
u32 FATStorage::ReadSectorsInternal(RFILE* file, u64 filelen, u32 start, u32 num, u8* data)
{
    if (!file) return 0;
    ...
    rfseek(file, addr, SEEK_SET);

    u32 res = rfread(data, 0x200, num, file);
    if (res < num)
    {
        if (feof(file))
        {
...
u32 FATStorage::WriteSectorsInternal(RFILE* file, u64 filelen, u32 start, u32 num, u8* data)
{
    if (!file) return 0;
    ...
    u32 res = rfwrite(data, 0x200, num, file);
    return res;
}
...
bool FATStorage::Load(std::string filename, u64 size, std::string sourcedir)
{
    ...
    else
    {
        ...
        if (FileSize == 0)
        {
            rfseek(FF_File, 0, SEEK_END);
            FileSize = rfseek(FF_File, 0, SEEK_CUR);
        }
    }
