...
#ifdef WRC
  double cyclesThisFrame = gSystemCycleCount - lynxie->mMikie->startTS;
  double refreshRate = (round(16000000.0 / cyclesThisFrame) * 10) / 10.0;

  if (lastRate != refreshRate) {
	lastRate = refreshRate;
	EM_ASM({
