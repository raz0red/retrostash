...
static void update_input(void)
{
   ...
   if (controller & INP_B) input_buf[0] |= (1 << 0); // RETRO_DEVICE_ID_JOYPAD_A,
   if (controller & INP_X) input_buf[0] |= (1 << 0); // RETRO_DEVICE_ID_JOYPAD_A,
   if (controller & INP_A) input_buf[0] |= (1 << 1); // RETRO_DEVICE_ID_JOYPAD_B,
   if (controller & INP_Y) input_buf[0] |= (1 << 1); // RETRO_DEVICE_ID_JOYPAD_B,
   if (controller & INP_RBUMP) input_buf[0] |= (1 << 2); // RETRO_DEVICE_ID_JOYPAD_L,
   if (controller & INP_LBUMP) input_buf[0] |= (1 << 3); // RETRO_DEVICE_ID_JOYPAD_R,

   if (controller & INP_LEFT) input_buf[0] |= (1 << 4); // RETRO_DEVICE_ID_JOYPAD_LEFT,
   if (controller & INP_RIGHT) input_buf[0] |= (1 << 5); // RETRO_DEVICE_ID_JOYPAD_RIGHT,
   if (controller & INP_UP) input_buf[0] |= (1 << 6); // RETRO_DEVICE_ID_JOYPAD_UP,
   if (controller & INP_DOWN) input_buf[0] |= (1 << 7); // RETRO_DEVICE_ID_JOYPAD_DOWN,

   if (controller & INP_START) input_buf[1] |= (1 << 0); // RETRO_DEVICE_ID_JOYPAD_START,
