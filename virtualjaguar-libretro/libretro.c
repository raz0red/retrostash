...
#if defined(WRC) && VJ_WRC_CLOCK_SCALE_ENABLED
EM_JS(int, wrc_get_m68k_clock_scale_pct, (), {
  try { return window.emulator.getM68kClockScalePct(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_m68k_clock_scale_pct threw:", e); return 100; }
});
EM_JS(int, wrc_get_risc_clock_scale_pct, (), {
  try { return window.emulator.getRiscClockScalePct(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_risc_clock_scale_pct threw:", e); return 100; }
});
#endif

#ifdef WRC
/* Pulled directly from JS once at load (editor property, launch-only --
 * same reasoning as the clock-scale pulls above: no options menu exists
 * to drive the normal environ_cb path for WRC builds). */
EM_JS(int, wrc_get_use_real_bios, (), {
  try { return window.emulator.getUseRealBios() ? 1 : 0; }
  catch (e) { console.error("WRC_DEBUG: wrc_get_use_real_bios threw:", e); return 0; }
});
#endif
...
#ifndef WRC
   /* WRC drives vjs.useFastBlitter directly from wrc_on_set_options()
    * (pushed from JS, initial load and any future in-game toggle) instead
    * of the environ_cb/config path -- skip this read so it doesn't stomp
    * that with the compiled-in core-option default. */
...
#elif VJ_WRC_CLOCK_SCALE_ENABLED
   /* WRC: pulled directly from JS once at load (editor property, not a
    * live/session toggle) -- no options menu exists to drive the normal
    * environ_cb path, and there's no need to round-trip a 5-way value
    * through OPT bits when JS can just hand back the number directly. */
   m68kClockScalePct = (uint32_t)wrc_get_m68k_clock_scale_pct();
#else
   /* VJ_WRC_CLOCK_SCALE_ENABLED is 0 -- see jaguar.h. Hardcoded stock;
    * the (UI-hidden) editor prop is ignored entirely. */
   m68kClockScalePct = 100;
#endif
...
#elif VJ_WRC_CLOCK_SCALE_ENABLED
   /* WRC: pulled directly from JS once at load, same reasoning as
    * m68kClockScalePct above. */
   riscClockScalePct = (uint32_t)wrc_get_risc_clock_scale_pct();
#else
   /* VJ_WRC_CLOCK_SCALE_ENABLED is 0 -- see jaguar.h. Hardcoded stock;
    * the (UI-hidden) editor prop is ignored entirely. */
   riscClockScalePct = 100;
...
#else
   vjs.useJaguarBIOS = wrc_get_use_real_bios() ? true : false;
#endif
...
#else
   /* Shares the single cart useRealBios flag rather than pulling a
    * separate CD-only tri-state -- the core's own "Auto" CD boot mode is
    * currently identical to "Real BIOS" (see libretro_core_options.h),
    * so a 3-way editor control never actually distinguished a third
    * behavior. */
   vjs.cdBootMode = wrc_get_use_real_bios() ? CDBOOT_BIOS : CDBOOT_HLE;
#endif
...
#ifdef WRC
/* Maps the WRC abstract pad (wrc_input_state) directly onto the Jaguar's
 * D-pad + A/B/C fire buttons + Pause/Option, bypassing RetroArch's
 * input_state_cb entirely. The jag_retropad remap options are not
 * reachable through WRC, so this only covers standard gameplay input.
 *
 * The numpad (12 keys: 1-9, 0, *, #) has no dedicated bits in wrc.h's
 * generic INP_* contract (it's a standard-gamepad-shaped bitfield, no
 * console this WRC targets before Jaguar has a phone-style keypad), so
 * it's packed into the 6 otherwise-unused bits 10-15
 * (INP_LBUMP/LTRIG/LTHUMB/RBUMP/RTRIG/RTHUMB) as a single 6-bit "active
 * key" value instead of one-hot flags -- JS only ever has at most one
 * keypad key mapped-and-held at a time, so a packed ID (0=none, 1-9=
 * digits, 10=0, 11=*, 12=#) is simpler than 12 more one-hot bits would
 * be. See webrcade-app-retro-virtual-jaguar's sendInput() override for
 * the JS-side packing. */
static void wrc_update_input(void)
{
   static const int keypad_button[12] = {
      BUTTON_1, BUTTON_2, BUTTON_3, BUTTON_4, BUTTON_5, BUTTON_6,
      BUTTON_7, BUTTON_8, BUTTON_9, BUTTON_0, BUTTON_s, BUTTON_d
   };
   unsigned player;

   for (player = 0; player < 2; player++)
   {
      unsigned int state = wrc_input_state[player];
      unsigned int keypad = (state >> 10) & 0x3F;

      if (state & INP_UP)     joypad_buttons[player][BUTTON_U] = 0xff;
      if (state & INP_DOWN)   joypad_buttons[player][BUTTON_D] = 0xff;
      if (state & INP_LEFT)   joypad_buttons[player][BUTTON_L] = 0xff;
      if (state & INP_RIGHT)  joypad_buttons[player][BUTTON_R] = 0xff;
      if (state & INP_A)      joypad_buttons[player][BUTTON_A] = 0xff;
      if (state & INP_B)      joypad_buttons[player][BUTTON_B] = 0xff;
      if (state & INP_Y)      joypad_buttons[player][BUTTON_C] = 0xff;
      if (state & INP_SELECT) joypad_buttons[player][BUTTON_PAUSE] = 0xff;
      if (state & INP_START)  joypad_buttons[player][BUTTON_OPTION] = 0xff;

      if (keypad >= 1 && keypad <= 12)
         joypad_buttons[player][keypad_button[keypad - 1]] = 0xff;
   }
}

/* Hooks called into the core from the WRC/emscripten frontend
 * (RetroArch/frontend/drivers/platform_emscripten.c and the per-core JS
 * glue). Unused for now; present so linking/exporting succeeds. */

/* OPT1: "Disable Fast Blitter" setting -- inverted so that no options set
 * (the default) means Fast, not Accurate. Pushed from JS whenever it
 * actually changes (initial load, and a future in-game settings toggle)
 * rather than polled every frame -- BlitterWriteWord() already reads
 * vjs.useFastBlitter fresh on every blit dispatch, so this flag just needs
 * to be current when a blit happens, not re-derived from JS on a timer.
 * Trades rendering accuracy for speed; see blitter_blit() vs
 * BlitterMidsummer2() in src/tom/blitter.c.
 * check_variables() must not also touch vjs.useFastBlitter, or its
 * environ_cb read during retro_load_game() (which runs after this can
 * already have fired once, before Module.callMain()) would stomp it back
 * to the compiled-in (upstream, Accurate) default. */
void wrc_on_set_options(int opts) {
   vjs.useFastBlitter = (opts & OPT1) ? false : true;
}
...
/* Stubbed for now (see a5200's libretro.c for precedent) -- RetroArch's
 * generic SRAM autosave still writes game.srm from retro_get_memory_data
 * independently of this hook, and eeprom_save_buf is kept live via
 * eeprom_dirty_cb, so saves aren't lost, just not force-flushed on demand.
 * TODO: wire this up like picodrive-official's em_cmd_savefiles() before
 * shipping, so the explicit save-sync path grabs fresh EEPROM state. */
void em_cmd_savefiles() {}

/* Only the five entry points RetroArch/frontend/drivers/platform_emscripten.c
 * actually links against are stubbed; the rest of chd.h's declarations are
 * unused. Jaguar CD reads CUE/BIN/CDI directly and never touches libchdr --
 * these exist purely to satisfy the linker for the cheevos CD-hashing code
 * that every emscripten core build pulls in unconditionally. */
...
chd_error chd_open(const char *filename, int mode, chd_file *parent, chd_file **chd) { return 0; }
