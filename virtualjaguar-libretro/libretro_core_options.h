...
struct retro_core_option_v2_definition option_defs_us[] = {
   {
      ...
      /* Tried defaulting this to "enabled" (Fast) on the emscripten/WRC
       * build as a best-case perf test -- confirmed visible rendering
       * breakage (torn/dashed sprite graphics), so back to upstream's
       * "disabled" (Accurate) default. TODO: expose this as a per-game
       * JS-side toggle (via wrc_on_set_options) instead of a build-time
       * default, so games that don't rely on the pixel-comparator/
       * Z-buffer precision can opt into the speed. */
      "disabled"
   },
   {
      ...
      /* Pure overhead with no gameplay value once a build is stable; WRC
       * has no core-options UI to flip this per game, so default it off
       * on the emscripten/WRC build specifically rather than eating the
       * watchdog cost on every frame in production. */
