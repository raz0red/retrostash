...
#ifdef WRC
   /* --- WRC / Xbox-style input mapping --- */
   int controller = wrc_input_state[0];

   // D-Pad (same order as RA map)
   if (controller & INP_UP)
      input_buf |= (1 << 0); // RETRO_DEVICE_ID_JOYPAD_UP

   if (controller & INP_DOWN)
      input_buf |= (1 << 1); // RETRO_DEVICE_ID_JOYPAD_DOWN

   if (controller & INP_LEFT)
      input_buf |= (1 << 2); // RETRO_DEVICE_ID_JOYPAD_LEFT

   if (controller & INP_RIGHT)
      input_buf |= (1 << 3); // RETRO_DEVICE_ID_JOYPAD_RIGHT

   if (controller & (INP_B | INP_X))
      input_buf |= (1 << 5); // RETRO_DEVICE_ID_JOYPAD_B

   if (controller & (INP_A | INP_Y))
      input_buf |= (1 << 4); // RETRO_DEVICE_ID_JOYPAD_A

   // Menu (≡) → Start
   if (controller & INP_START)
      input_buf |= (1 << 6); // RETRO_DEVICE_ID_JOYPAD_START

#else
   /* --- RetroArch / libretro input mapping --- */
   static unsigned map[] = {
      RETRO_DEVICE_ID_JOYPAD_UP,    /* 0 */
      RETRO_DEVICE_ID_JOYPAD_DOWN,  /* 1 */
      RETRO_DEVICE_ID_JOYPAD_LEFT,  /* 2 */
      RETRO_DEVICE_ID_JOYPAD_RIGHT, /* 3 */
      RETRO_DEVICE_ID_JOYPAD_B,     /* 4 */
