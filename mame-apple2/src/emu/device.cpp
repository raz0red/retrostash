...
void device_t::start()
{
	...
	// reset deferred flag before all start steps
	m_start_deferred = false;
	...
	// if an interface deferred (Emscripten: can't throw), bail out without marking started
	if (m_start_deferred)
	{
		printf("WRC: '%s' deferred in interface_pre_start\n", tag());
		return;
	}
	...
	// if device deferred its start (Emscripten: can't throw), bail out without marking started
	printf("WRC: device_t::start() post device_start for '%s' deferred=%d\n", tag(), m_start_deferred ? 1 : 0);
	if (m_start_deferred)
		return;
