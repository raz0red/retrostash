...
/* Apple II RetroAchievements support - can be disabled if issues arise */
#define ENABLE_RETROACHIEVEMENTS 1

#if ENABLE_RETROACHIEVEMENTS
#include "machine/ram.h"  /* For direct RAM access */
#endif
...
#if ENABLE_RETROACHIEVEMENTS
/* Try to get direct pointer to Apple IIe RAM (much safer than reading from address space) */
static uint8_t* get_apple2_ram_pointer(size_t *out_size = nullptr)
{
   if (!mame_machine_manager::instance() || !mame_machine_manager::instance()->machine())
      return nullptr;

   /* Try to find the RAM device and get its pointer directly */
   auto &root = mame_machine_manager::instance()->machine()->root_device();
   device_t *ram_dev = root.subdevice("ram");
   if (ram_dev)
   {
      /* Cast to ram_device and get pointer - this is MUCH safer than reading from address space! */
      ram_device *ram = dynamic_cast<ram_device*>(ram_dev);
      if (ram)
      {
         if (out_size)
            *out_size = ram->size();
...
void *retro_get_memory_data(unsigned type)
{
...
#if ENABLE_RETROACHIEVEMENTS
   /* Apple II: Return direct pointer to RAM (no copying needed) */
   if (     type == RETRO_MEMORY_SYSTEM_RAM
         && mame_machine_manager::instance() != NULL
         && mame_machine_manager::instance()->machine() != NULL)
   {
      size_t ram_size = 0;
      uint8_t *ram_ptr = get_apple2_ram_pointer(&ram_size);
      if (ram_ptr)
      {
         if (!logged)
         {
            printf("[CHEEVOS] Returning direct RAM pointer at %p, size=%u bytes\n", ram_ptr, (unsigned)ram_size);
            logged = true;
         }
         return ram_ptr;
      }
         ...
         && mame_machine_manager::instance()->machine() != NULL)
   {
         ...
         for (space_index = 0; space_index < memory.num_spaces(); space_index++)
         {
            if (memory.has_space(space_index))
            {
               ...
               for (address_map_entry &entry : space.map()->m_entrylist)
               {
                  entry_count++;
                  if (!logged)
                     log_cb(RETRO_LOG_INFO, "[CHEEVOS] Entry %d: $%06X-$%06X read_type=%d write_type=%d\n",
                        entry_count, (unsigned)entry.m_addrstart, (unsigned)entry.m_addrend,
                        entry.m_read.m_type, entry.m_write.m_type);

                  if (entry.m_read.m_type == AMH_RAM)
                  {
                     ram_entry_count++;
                     if (!logged)
                        log_cb(RETRO_LOG_INFO, "[CHEEVOS] RAM entry: $%06X-$%06X read=%d write=%d share=%p\n",
                           (unsigned)entry.m_addrstart, (unsigned)entry.m_addrend,
                           entry.m_read.m_type, entry.m_write.m_type, entry.m_share);
      ...
      if (!logged)
      {
         log_cb(RETRO_LOG_INFO, "[CHEEVOS] Memory scan: %d devices, %d spaces, %d entries, %d RAM entries\n",
            device_count, space_count, entry_count, ram_entry_count);
         logged = true;
      }
   }
...
#if ENABLE_RETROACHIEVEMENTS
   /* Apple II: Return direct RAM size */
   if (     type == RETRO_MEMORY_SYSTEM_RAM
         && mame_machine_manager::instance() != NULL
         && mame_machine_manager::instance()->machine() != NULL)
   {
      size_t ram_size = 0;
      if (get_apple2_ram_pointer(&ram_size))
         return ram_size;
   }
#endif
