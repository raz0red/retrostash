...
void retro_main_loop()
{
	...
	if (ENDEXEC)
	{
		...
		printf("[WRC] emu_fatalerror in retro_loop: %s\n", err.what());
	} catch (device_missing_dependencies const &) {
		printf("[WRC] device_missing_dependencies in retro_loop!\n");
	} catch (std::exception const &ex) {
		printf("[WRC] std::exception in retro_loop: %s\n", ex.what());
	} catch (...) {
		printf("[WRC] unknown exception in retro_loop!\n");
	}
}
#endif
