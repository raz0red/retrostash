...
EM_JS(int, wrc_get_port2, (), {
  try { return window.emulator.getPort2(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_port2 threw:", e); return 0; }
});

EM_JS(int, wrc_is_3button, (), {
  try { return window.emulator.is3Button() ? 1 : 0; }
  catch (e) { console.error("WRC_DEBUG: wrc_is_3button threw:", e); return 0; }
});

EM_JS(int, wrc_get_mouse_x, (), {
  try { return window.emulator.getMouseX(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_mouse_x threw:", e); return 160; }
});

EM_JS(int, wrc_get_mouse_y, (), {
  try { return window.emulator.getMouseY(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_mouse_y threw:", e); return 112; }
});

EM_JS(int, wrc_get_mouse_buttons, (), {
  try { return window.emulator.getMouseButtons(); }
  catch (e) { console.error("WRC_DEBUG: wrc_get_mouse_buttons threw:", e); return 0; }
});

EM_JS(void, wrc_report_non_32x_content, (), {
  try { window.emulator.reportNon32XContent(); }
  catch (e) { console.error("WRC_DEBUG: wrc_report_non_32x_content threw:", e); }
});

// Whether port 2 (index 1) is currently configured as a light gun, so
// retro_run() doesn't need to call back into JS every frame to check.
static int wrc_port2_is_lightgun;

// One-time boot validation. PicoIn.AHW & PAHW_32X is the direct observable
// side effect of Pico32xStartup() (it's the very first thing that function
// sets) - checking it here is equivalent to "was Pico32xStartup() called"
// without touching core pico/ code. Real 32X/32X CD content always reaches
// this within a couple seconds of boot via the same MARS-probe/ADEN-write
// sequence real hardware uses. Video and audio output are suppressed for
// as long as this is unvalidated, so nothing from non-32X content is ever
// shown or heard. This is a ONE-TIME window, not a per-frame requirement:
// PAHW_32X legitimately toggles off during normal mid-game 32X program
// reloads (see Pico32xShutdown()), so once validated we never check again.
#define WRC_32X_BOOT_WINDOW_FRAMES 180 // ~3 seconds at 60fps
static int wrc_32x_validated;
static int wrc_32x_failed;
static int wrc_32x_boot_frames;

// Independent of 32X validation: hold audio muted for the first ~1s of
// every game load regardless. The engine/AudioContext produces an audible
// pop on the very first real audio it receives after a fresh load - this
// reproduces even with no other WRC changes at all, so it's unrelated to
// the mute/unmute transition above. Muting briefly at cold start avoids it.
#define WRC_AUDIO_STARTUP_MUTE_FRAMES 60
static int wrc_audio_mute_frames_left;
#endif
...
#ifdef WRC
// Applies webrcade's port1/port2 device config, the same place a real
// RetroArch session would apply the picodrive_input1/2 core options (see
// update_variables() below). Called once at load and once per frame from
// retro_run(), but only actually calls PicoSetInputDevice() when something
// changed - PicoSetInputDevice() resets per-port TH-toggle timing state
// (padTHTimeout etc.), so calling it unconditionally every frame would
// permanently reset the pad ID/light-gun read protocol's phase state.
static void wrc_apply_input_devices(void)
{
   static int wrc_last_port2 = -1;
   static enum input_device wrc_last_pad_type = -1;

   int port2 = wrc_get_port2();
   enum input_device pad_type = wrc_is_3button() ? PICO_INPUT_PAD_3BTN : PICO_INPUT_PAD_6BTN;

   if (port2 == wrc_last_port2 && pad_type == wrc_last_pad_type)
      return;
   wrc_last_port2 = port2;
   wrc_last_pad_type = pad_type;

   enum input_device dev1 = pad_type;
   if (port2 == 10)
      dev1 = PICO_INPUT_NOTHING;
   else if (port2 == 1)
      dev1 = PICO_INPUT_LIGHT_GUN;

   PicoSetInputDevice(0, pad_type);
   PicoSetInputDevice(1, dev1);
   wrc_port2_is_lightgun = (port2 == 1);
}

// Both cart SRAM and CD BRAM come through the same slot - see
// retro_get_memory_data()'s RETRO_MEMORY_SAVE_RAM case just below.
#define WRC_SRM_PATH "/home/web_user/retroarch/userdata/saves/game.srm"

// Called once, right after a game loads: if webrcade already wrote a
// previously-saved game.srm to the FS (via Emulator.loadState(), which runs
// before the wasm module starts), read it into the live save-RAM buffer.
static void wrc_load_srm(void)
{
...
#ifdef WRC
   wrc_apply_input_devices();
   wrc_load_srm();
   if (PicoIn.AHW & PAHW_MCD) {
      /* 32X CD's boot sequence (CD BIOS -> load boot ISO -> hand off to the
       * 32X program) doesn't reliably reach the ADEN write that flips on
       * PAHW_32X within the same boot window real 32X carts do, so this
       * validation isn't reliable for CD content. It's also a 6-game
       * catalog, so just skip the check there instead of tuning around it. */
      wrc_32x_validated = 1;
      wrc_32x_failed = 0;
      wrc_32x_boot_frames = 0;
   } else {
      wrc_32x_validated = 0;
      wrc_32x_failed = 0;
      wrc_32x_boot_frames = 0;
   }
   wrc_audio_mute_frames_left = WRC_AUDIO_STARTUP_MUTE_FRAMES;
#endif
...
{
#ifdef WRC
   /* Mirrors how pause/resume avoids a pop: while paused, RetroArch's core
    * loop just never calls into here at all - it doesn't feed the audio
    * pipeline zeroed data. Do the same here (skip the push entirely,
    * rather than writing silence through it) so the first real audio
    * after validation is the pipeline's first call ever, not a jump out
    * of a live zero-filled stream. */
   if (!wrc_32x_validated || wrc_audio_mute_frames_left > 0)
      return;
#endif
...
void retro_run(void)
{
...
#ifdef WRC
   // Map wrc_input_state directly onto the Genesis pad, bypassing the
   // libretro input_state_cb layer entirely.
   for (pad = 0; pad < padcount; pad++)
   {
      unsigned int wrc_state = wrc_input_state[pad];
      unsigned int p = 0;

      if (wrc_state & INP_UP)     p |= 1 << GBTN_UP;
      if (wrc_state & INP_DOWN)   p |= 1 << GBTN_DOWN;
      if (wrc_state & INP_LEFT)   p |= 1 << GBTN_LEFT;
      if (wrc_state & INP_RIGHT)  p |= 1 << GBTN_RIGHT;
      if (wrc_state & INP_START)  p |= 1 << GBTN_START;
      if (wrc_state & INP_SELECT) p |= 1 << GBTN_MODE;
      if (wrc_state & INP_X)      p |= 1 << GBTN_A;
      if (wrc_state & INP_A)      p |= 1 << GBTN_B;
      if (wrc_state & INP_B)      p |= 1 << GBTN_C;
      if (wrc_state & INP_LBUMP)  p |= 1 << GBTN_X;
      if (wrc_state & INP_Y)      p |= 1 << GBTN_Y;
      if (wrc_state & INP_RBUMP)  p |= 1 << GBTN_Z;

      PicoIn.pad[pad] = p;

      // Also populate the raw RA-bit-numbered input[] array (1:1 by CID
      // name, not the Genesis remap above), since it's still consumed
      // below for Sega Pico pen handling.
      if (wrc_state & INP_B)      input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_B;
      if (wrc_state & INP_Y)      input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_Y;
      if (wrc_state & INP_SELECT) input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_SELECT;
      if (wrc_state & INP_START)  input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_START;
      if (wrc_state & INP_UP)     input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_UP;
      if (wrc_state & INP_DOWN)   input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_DOWN;
      if (wrc_state & INP_LEFT)   input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_LEFT;
      if (wrc_state & INP_RIGHT)  input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_RIGHT;
      if (wrc_state & INP_A)      input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_A;
      if (wrc_state & INP_X)      input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_X;
      if (wrc_state & INP_LBUMP)  input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_L;
      if (wrc_state & INP_RBUMP)  input[pad] |= 1 << RETRO_DEVICE_ID_JOYPAD_R;
   }
#else
   ...
   if (wrc_port2_is_lightgun) {
      int mx = wrc_get_mouse_x();
      int my = wrc_get_mouse_y();
      int mb = wrc_get_mouse_buttons();
      static int wrc_last_mb;

      PicoIn.mouse[0] = mx;
      PicoIn.mouse[1] = my;

      if (mb & 1) PicoIn.pad[1] |= 1 << GBTN_A;     // trigger, as Sega Menacer
      if (mb & 2) PicoIn.pad[1] |= 1 << GBTN_B;
      if (mb & 4) PicoIn.pad[1] |= 1 << GBTN_START;

      if (mb != wrc_last_mb) {
         printf("WRC_DEBUG: gun mx=%d my=%d mb=%d pad[1]=0x%04x\n", mx, my, mb, PicoIn.pad[1]);
         wrc_last_mb = mb;
      }
   }
#endif
...
#ifdef WRC
   /* One-time boot validation that this is actually 32X content. See the
    * comment above wrc_32x_validated's declaration for the full rationale. */
   if (!wrc_32x_validated && !wrc_32x_failed) {
      if (PicoIn.AHW & PAHW_32X) {
         wrc_32x_validated = 1;
      } else if (++wrc_32x_boot_frames >= WRC_32X_BOOT_WINDOW_FRAMES) {
         wrc_32x_failed = 1;
         wrc_report_non_32x_content();
      }
   }

   if (wrc_audio_mute_frames_left > 0)
      wrc_audio_mute_frames_left--;
#endif
   ...
   if (wrc_port2_is_lightgun) {
      pico_inp_mode = 2;
      pico_pen_x = PicoIn.mouseInt[0];
      pico_pen_y = PicoIn.mouseInt[1];
      draw_pico_ptr();
   }
#endif
...
#ifdef WRC
// Called from JS (Emulator.saveState(), via Module._cmd_savefiles()) right
// before it reads game.srm back off the FS - flushes the live save-RAM
// buffer (cart SRAM or CD BRAM, whichever is active - see
// retro_get_memory_data()'s RETRO_MEMORY_SAVE_RAM case) out to that path.
...
int wrc_start(char* arg) { return 0; }
#endif
