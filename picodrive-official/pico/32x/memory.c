...
{
  u32 d = 0;
  static int mars_probe_count;

  if ((a & 0xfffc) == 0x30ec && mars_probe_count < 20) {
    printf("WRC_DEBUG: PicoRead8_32x MARS probe a=%06x opt_en_32x=%d\n",
          a, !!(PicoIn.opt & POPT_EN_32X));
    mars_probe_count++;
  }
...
u32 PicoRead16_32x(u32 a)
{
  ...
  if ((a & 0xfffc) == 0x30ec && mars_probe_count < 20) {
    printf("WRC_DEBUG: PicoRead16_32x MARS probe a=%06x opt_en_32x=%d\n",
          a, !!(PicoIn.opt & POPT_EN_32X));
    mars_probe_count++;
  }
