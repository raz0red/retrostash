...
#ifdef WRC
  unsigned int state = wrc_input_state[port_];
  jp.u  = (state & INP_UP) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_UP);
  jp.d  = (state & INP_DOWN) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_DOWN);
  jp.l  = (state & INP_LEFT) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_LEFT);
  jp.r  = (state & INP_RIGHT) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_RIGHT);
  jp.lt = (state & INP_LBUMP) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_L);
  jp.rt = (state & INP_RBUMP) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_R);
  jp.x  = (state & INP_SELECT) ? 1 : 0; // poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_SELECT);
  jp.p  = (state & INP_START || state & INP_Y) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_START) |
  jp.a  = (state & INP_X) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_Y);
  jp.b  = (state & INP_A) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_B);
  jp.c  = (state & INP_B) ? 1 : 0; //poll_joypad(port_,RETRO_DEVICE_ID_JOYPAD_A);
#else
