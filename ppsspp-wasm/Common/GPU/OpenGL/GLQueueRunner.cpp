...
#ifdef WRC
				float near = c.viewport.vp.minZ;
				float far = c.viewport.vp.maxZ;

				// FIX for WebGL: zNear cannot be greater than zFar
				if (near > far) {
					float temp = near;
					near = far;
					far = temp;
