...
// WRC - System_AudioPushSamples() (below) appends into output_audio_buffer
// from the real emu pthread during HLE audio mixing, while
// Libretro::upload_output_audio_buffer() reads/resets it from the main/video
// thread once per retro_run() tick. Unguarded, this is a real data race -
// ensure_output_audio_buffer_capacity() can even realloc() the buffer out
// from under a concurrent memcpy on the other thread. This mutex serializes
// the two sides.
static std::mutex g_audioBufferMutex;
...
#ifdef WRC
      // WRC - bypass RetroArch's own audio driver (rwebaudio) entirely,
      // same pattern as fceumm/snes9x/etc: call straight into the JS-side
      // circular-buffer audio processor (window.emulator.audioCallback),
      // which pulls samples on the browser's own audio-hardware clock
      // instead of rwebaudio's explicit AudioBufferSourceNode.start(when)
      // scheduling chain - the latter breaks (audible pop) if this thread
      // is ever late by even one buffer, since a start() time in the past
      // just gets clamped to "now" with a gap. The ring buffer's slewing
      // logic absorbs push-timing jitter instead.
...
#ifdef WRC
   // WRC - wrc_input_state[0]/wrc_input_state_analog[0] are populated every
   // frame by wrc_set_input() (frontend/drivers/platform_emscripten.c,
   // shared across every WRC core - not something this core defines
   // itself), driven by RetroAppWrapper's own (un-overridden) generic
   // pollControls()/sendInput() - see webrcade-app-common's
   // app/retrowrapper/index.js. Translated here into the same
   // RETRO_DEVICE_ID_JOYPAD_* bit positions the map[] table below already
   // expects, so the rest of this function is untouched. Face buttons use
   // standard-gamepad position (not letter match): WRC's A/B/X/Y
   // (bottom/right/left/top) -> libretro's B/A/Y/X, which map[] below
   // already sends to Cross/Circle/Square/Triangle respectively. L/R
   // accept either the shoulder-bumper or analog-trigger WRC bit, since
   // PSP has only one digital L/R button (no separate bumper vs. trigger
   // the way a real gamepad or PSX does).
   {
      unsigned int w = wrc_input_state[0];
      if (w & INP_UP)     ret |= (1 << RETRO_DEVICE_ID_JOYPAD_UP);
      if (w & INP_DOWN)   ret |= (1 << RETRO_DEVICE_ID_JOYPAD_DOWN);
      if (w & INP_LEFT)   ret |= (1 << RETRO_DEVICE_ID_JOYPAD_LEFT);
      if (w & INP_RIGHT)  ret |= (1 << RETRO_DEVICE_ID_JOYPAD_RIGHT);
      if (w & INP_START)  ret |= (1 << RETRO_DEVICE_ID_JOYPAD_START);
      if (w & INP_SELECT) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_SELECT);
      if (w & INP_A) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_B); // Cross
      if (w & INP_B) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_A); // Circle
      if (w & INP_X) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_Y); // Square
      if (w & INP_Y) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_X); // Triangle
      if (w & (INP_LBUMP | INP_LTRIG)) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_L);
      if (w & (INP_RBUMP | INP_RTRIG)) ret |= (1 << RETRO_DEVICE_ID_JOYPAD_R);
   }
#else
...
#ifdef WRC
   // WRC - wrc_input_state_analog[0] is already normalized to [-1,1] (no
   // /32767 division needed) in the raw HTML5 Gamepad API Y-axis
   // convention (negative = up, matching controllers.getAxisValue()) -
   // negated to match this function's own convention below (it divides
   // libretro's raw analog value by -32767.0f for the same reason). Index
   // 2/3 (right stick) is PPSSPP's own real second-analog-stick input
   // (CTRL_STICK_RIGHT below) - no button-based "twin stick" emulation
   // needed, unlike some other consoles' second-stick conventions.
   float x_left  = wrc_input_state_analog[0][0];
   float y_left  = -wrc_input_state_analog[0][1];
   float x_right = wrc_input_state_analog[0][2];
   float y_right = -wrc_input_state_analog[0][3];
#else
