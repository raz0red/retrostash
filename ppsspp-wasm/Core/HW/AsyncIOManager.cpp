...
void AsyncIOManager::ScheduleOperation(const AsyncIOEvent &ev) {
	...
	// WRC - process synchronously instead of queuing for the background I/O
	// thread. WaitResult()/ResultFinishTicks() below block on resultsWait_
	// waiting for that thread to signal completion via ScheduleEvent(); under
	// this build's threading model that signal doesn't reliably arrive,
	// making those waits hang indefinitely - confirmed via a real lockup
	// right after sceUtilitySavedataInitStart(AUTOLOAD), which uses this
	// exact async-I/O path to read the save file. Doing the work immediately
	// here means results_ is already populated by the time WaitResult() is
	// called, so its own #ifdef __EMSCRIPTEN__ branch below never actually
	// needs to wait.
	ProcessEvent(ev);
#else
	ScheduleEvent(ev);
...
#ifdef __EMSCRIPTEN__
		// WRC - see ScheduleOperation()'s comment. Never block here: the
		// background thread this would be waiting on doesn't reliably wake
		// it under this build. ProcessEvent() already ran synchronously
		// when the operation was scheduled, so reaching here with no
		// result means the operation is genuinely not ready yet (or won't
		// ever complete this way) - return false rather than hang.
		return false;
#else
...
#ifdef __EMSCRIPTEN__
		// WRC - see ScheduleOperation()'s comment - never block here.
		return 0;
#else
		resultsWait_.wait_for(guard, std::chrono::milliseconds(16));
#endif
