...
#ifdef WRC
/* mupen64plus-core/src/plugin/ -> retrostash/wrc.h. Verify this depth
   against the actual build tree; a wrong count just fails to find the
   header at compile time and is a one-line fix. */
#include "../../../../wrc.h"
#endif
...
#ifdef WRC
/* WRC input is fed via wrc_set_input() (frontend/drivers/platform_emscripten
   .c) into the raw wrc_input_state[]/wrc_input_state_analog[] globals, not
   through the standard input_cb polling callback - self-contained, doesn't
   reuse inputGetKeys_reuse() below (which reads from input_cb and also owns
   dpad/start/the SELECT-held game-profile-switch check, neither of which
   apply here). Bit layout matches the JS-side N64KeyCodeToControlMapping /
   pollControls() convention in webrcade-app-retro-parallel-n64, and mirrors
   mupen64plus-libretro-nx/custom/mupen64plus-core/plugin/
   emulate_game_controller_via_libretro.c's own inputGetKeys_wrc exactly. */
static int WRC_UP      = 0x0001;
static int WRC_DOWN    = 0x0002;
static int WRC_LEFT    = 0x0004;
static int WRC_RIGHT   = 0x0008;
static int WRC_START   = 0x0010;
static int WRC_R_KEY   = 0x0020;
static int WRC_L_KEY   = 0x0040;
static int WRC_Z_KEY   = 0x0080;
static int WRC_A_KEY   = 0x0100;
static int WRC_B_KEY   = 0x0200;
static int WRC_CL_KEY  = 0x0400;
static int WRC_CR_KEY  = 0x0800;
static int WRC_CU_KEY  = 0x1000;
static int WRC_CD_KEY  = 0x2000;

static void inputGetKeys_wrc( int Control, BUTTONS *Keys )
{
   unsigned int wrc_state = wrc_input_state[Control];
   double radius, angle;
   int16_t analogX = (int16_t)(wrc_input_state_analog[Control][0] * 0x7fff);
   int16_t analogY = (int16_t)(wrc_input_state_analog[Control][1] * 0x7fff);
   Keys->Value = 0;

   Keys->R_DPAD       = !!(wrc_state & WRC_RIGHT);
   Keys->L_DPAD       = !!(wrc_state & WRC_LEFT);
   Keys->D_DPAD       = !!(wrc_state & WRC_DOWN);
   Keys->U_DPAD       = !!(wrc_state & WRC_UP);
   Keys->START_BUTTON = !!(wrc_state & WRC_START);
   ...
   Keys->Z_TRIG       = !!(wrc_state & WRC_Z_KEY);
   Keys->L_TRIG       = !!(wrc_state & WRC_L_KEY);
   Keys->R_TRIG       = !!(wrc_state & WRC_R_KEY);
   Keys->A_BUTTON     = !!(wrc_state & WRC_A_KEY);
   Keys->B_BUTTON     = !!(wrc_state & WRC_B_KEY);

   if (wrc_state & WRC_CU_KEY) Keys->Value |= CSTICK_UP;
   if (wrc_state & WRC_CD_KEY) Keys->Value |= CSTICK_DOWN;
   if (wrc_state & WRC_CL_KEY) Keys->Value |= CSTICK_LEFT;
   if (wrc_state & WRC_CR_KEY) Keys->Value |= CSTICK_RIGHT;

   /* N64 control stick - left analog stick */
   radius = sqrt((double)analogX * analogX + (double)analogY * analogY);
   angle = atan2((double)analogY, (double)analogX);
