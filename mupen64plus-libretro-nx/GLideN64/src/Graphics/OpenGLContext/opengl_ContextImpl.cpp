...
void ContextImpl::init()
{
	...
	{
#ifdef WRC
		/* Tried forcing BufferedDrawer (plain glBufferData() VBO path,
		   since it already falls back correctly without bufferStorage -
		   see opengl_BufferedDrawer.cpp) for a performance win over
		   UnbufferedDrawer. Caused a full black screen in testing -
		   reverted to upstream's condition (== UnbufferedDrawer under
		   WebGL) until root-caused further. */
		const bool useBufferedDrawer = !m_glInfo.isGLESX || (m_glInfo.bufferStorage && m_glInfo.drawElementsBaseVertex);
#elif defined(FORCE_UNBUFFERED_DRAWER)
		const bool useBufferedDrawer = false;
#else
		const bool useBufferedDrawer = !m_glInfo.isGLESX || (m_glInfo.bufferStorage && m_glInfo.drawElementsBaseVertex);
#endif
		if (useBufferedDrawer)
