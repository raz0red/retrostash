...
// Per-fiber Asyncify unwind/rewind capture buffer, passed to
// emscripten_fiber_init/_from_current_context. The default 131072 (128KB)
// is too small for this core's deep call chains under whole-program
// Asyncify instrumentation (interpreter -> plugin -> GLideN64 -> GL driver);
// overflow here doesn't always trap, it can silently corrupt adjacent heap
// memory, which explains symptoms ranging from garbled/missing video to
// hard "unreachable" crashes on deeper titles like Donkey Kong 64. Matches
// the -s ASYNCIFY_STACK_SIZE=5242880 linker flag in the dedicated
// Makefile.emscripten.mupen64plus-next for consistency.
#define ASYNCIFY_STACK_SIZE (5242880)
