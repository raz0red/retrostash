...
#ifndef WRC
    // Registers raw pointers (dev->rdram.dram, etc.) with RetroArch via
    // RETRO_ENVIRONMENT_SET_MEMORY_MAPS. Under Emscripten with
    // ALLOW_MEMORY_GROWTH=1, a heap-growth event after this call
    // reallocates the underlying ArrayBuffer, leaving these registered
    // pointers stale - suspected cause of memory-hungry-game corruption.
