...
bool CartDetector::isProbablySC(const ByteBuffer& image, size_t size)
{
  ...
  ptr = image.get();
  checksize = size;
  uInt8 firstchar = ptr[0];   //updated
  while(checksize)
  {
    // The dumper returns 0xFF on the write port (the first 128 bytes)
    // and returns random on the read port (the second 128 bytes)
    for (int t=0;t<128;t++)
      if (ptr[t]!=firstchar)  //updated
        return false;
