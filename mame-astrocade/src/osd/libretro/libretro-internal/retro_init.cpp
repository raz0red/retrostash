...
//    strcpy(MsystemName, "astrocde"); // Force Astrocade system
//    strcpy(mediaType, "-cart");      // Astrocade uses cartridges
//    if (log_cb) log_cb(RETRO_LOG_ERROR, "WRC: system=%s game=%s media=%s\n", MsystemName, MgameName, mediaType);

   printf("Path extraction result: File name = \"%s\"\n", srcpath);
   printf("Path extraction result: Game name = \"%s\"\n", MgameName);
   printf("Path extraction result: System name = \"%s\"\n", MsystemName);
   printf("Path extraction result: Game path = \"%s\"\n", MgamePath);
   printf("Path extraction result: Parent path = \"%s\"\n", MparentPath);
}
